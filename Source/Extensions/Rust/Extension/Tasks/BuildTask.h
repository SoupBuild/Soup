...
namespace Soup::Rust
{
	...
	private:
			...
			Soup::Build::Utilities::BuildStateWrapper& buildState)
		{
			...
			auto arguments = Soup::Rust::Compiler::BuildArguments();
			...
			arguments.TargetType = static_cast<Soup::Rust::Compiler::BuildTargetType>(
			...
			auto compiler = std::make_shared<Soup::Rust::Compiler::Compiler>(
				Path(roslynToolsRoot),
				Path("csc.exe"));

			auto buildEngine = Soup::Rust::Compiler::BuildEngine(compiler);
