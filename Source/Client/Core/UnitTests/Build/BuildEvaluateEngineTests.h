	...
	public:
		...
		void Execute_TwoOperations_DuplicateOutputFile_Fails()
		{
			...
			Assert::AreEqual<std::string_view>(
				"File \"C:/TestWorkingDirectory/OutputFile.out\" already written to by operation \"TestCommand: 2\"",
				exception.what(),
		...
		void Execute_TwoOperations_UndeclaredOutputWithDeclaredInput_Fails()
		{
			...
			Assert::AreEqual<std::string_view>(
				"File \"C:/TestWorkingDirectory/File.txt\" creates new dependency to existing declared inputs",
				exception.what(),
				"Verify Exception message");
